...
function data_out=static_solver_CTS_ORI(data)
...
for k=1:substep
    ...
    for i=1:1e3
        ...
        IF=Ki*X+phTpn*M_h;                   %unbalanced force

        Fp=w-IF;                                       %unbalanced force
        ...
        Kt_aa=Kg_aa+(Ke_aa+Ke_aa')/2;       % this is to 
           ...
           % jacobian matrix (of hinge)
            ...
            K_t_oa=Kt_aa+Ia'*(phTpn*diag(k_h)*phTpn'+G*kron(M_h,eye(3*nn)))*Ia;
...
%          dXa=(lmd*eye(size(Km)))\Fp_a;
        ...
        if use_energy==1
            ...
            [x,V]=fminbnd(@energy_CTS_ORI,0,1e1,opt);
        end
        Xa=Xa+x*dXa;
    end
    %
    ...
    data_out.M_out(:,k)=M_h;      % moment of hinge
