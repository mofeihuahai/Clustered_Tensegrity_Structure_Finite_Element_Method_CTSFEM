function data_out = ode4_CTS_membrane(odefun,tspan,y0,data_in)
...
global l_tc l_l l0_tc l0_l  sigma_l t_tc sigma_l_int n t_tc_int l_tc_int l_l_int n_d 
data_out=data_in;  %initialize output data
out_tspan=data_in.out_tspan;
ne=data_in.ne;
nn= data_in.nn;
np= data_in.np;
l_tc_int=data_in.l0_tc_t(:,1);           % initialize the intermidiate variable
l_l_int=data_in.l0_l_t(:,1);           
t_tc_int=zeros(size(data_in.l0_tc_t(:,1)));
sigma_l_int=zeros(3*np,1);
%%
if ~isnumeric(tspan)
...
data_out.t_tc_t=zeros(size(data_in.l0_tc_t(:,1),1),numel(out_tspan));
data_out.sigma_l_t=zeros(3*np,numel(out_tspan));
data_out.n_t=zeros(3*nn,numel(out_tspan));
data_out.l_tc_t=zeros(size(data_in.l0_tc_t(:,1),1),numel(out_tspan));
data_out.l_l_t=zeros(size(data_in.l0_l_t(:,1),1),numel(out_tspan));
...
%         data_out.strain_t(:,out_tspan==ti)=strain; 
        ...
        t_tc_int=t_tc;sigma_l_int=sigma_l; l_tc_int=l_tc;l_l_int=l_l;               %store the force and length(for plastic calculation)
