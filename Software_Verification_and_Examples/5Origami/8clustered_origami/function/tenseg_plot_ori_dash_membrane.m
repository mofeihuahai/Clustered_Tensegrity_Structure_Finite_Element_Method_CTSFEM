function [fig_out] = tenseg_plot_ori_dash_membrane( N,C_b,C_s,C_l,C_h,C_rh,fig_handle,highlight_nodes,view_vec, PlotTitle, R3Ddata,Ca)
...
LineWidth = 1; % Width of panel lines
...
FontLines = 18; % Font of panel line labels
...
if nargin < 8+2 % Empty Plot Title
    PlotTitle = '';
end
if nargin < 9+2 % No 3D object data
    R3Ddata = [];
end
...
if ~isempty(R3Ddata) % Extending axis to account for radii of objects
    ...
    if isfield(R3Ddata,'Lradius')
        maxradius = max(maxradius,max(R3Ddata.Lradius));
    end
...
%% Plot line member vectors
if ~isempty(C_l)
    A = N*C_l';
    line_start_nodes = zeros(3,size(A,2));
    line_end_nodes = zeros(3,size(A,2));
    for j = 1:size(A,2)
        line_start_nodes(:,j) = N(:,C_l(j,:)==-1);
        line_end_nodes(:,j) = N(:,C_l(j,:)==1);
    end
    
    if ~isempty(R3Ddata) && isfield(R3Ddata,'Lradius') % 3D plot
        for j = 1:size(C_l,1)
            [LatFace, UpFace, DwFace] = PlotCylinderObject(line_start_nodes(:,j),line_end_nodes(:,j),...
                R3Ddata.Lradius(j),nsurfpatches); % Lateral, upper, and down surface of cylinder representing a bar
            
            surf(LatFace.x, LatFace.y, LatFace.z, 'MeshStyle','row','FaceColor',LineSurfColor, ...
                'FaceLighting','gouraud', 'AmbientStrength',LightAmbientStrength);
            hold on
            surf(UpFace.x, UpFace.y, UpFace.z, 'MeshStyle','row','FaceColor',LineSurfColor, ...
                'FaceLighting','gouraud', 'AmbientStrength',LightAmbientStrength);
            hold on
            surf(DwFace.x, DwFace.y, DwFace.z, 'MeshStyle','row','FaceColor',LineSurfColor, ...
                'FaceLighting','gouraud', 'AmbientStrength',LightAmbientStrength);
            hold on
        end
    else % Normal line plot
        quiver3(line_start_nodes(1,:),line_start_nodes(2,:),line_start_nodes(3,:),A(1,:),A(2,:),A(3,:),'green.','Autoscale','off','LineWidth',LineWidth)
        hold on
    end
    % Write bar labels
    if LabelLines == 1
        for i = 1:size(A,2)
            text(line_start_nodes(1,i) + 0.5*A(1,i) + dist_x, ...
                line_start_nodes(2,i) + 0.5*A(2,i) + dist_y, ...
                line_start_nodes(3,i) + 0.5*A(3,i) + dist_z,...
                num2str(i), 'FontSize',FontLines, 'Color', 'k')
        end
    end
end
