...
% input data
data.N=N; data.C=C; data.C_l=C_l;data.C_t=C_t;data.C_pl_bar=C_pl_bar;data.U_t=U_t;data.R=R;data.C_pn_i=C_pn_i;data.np=np;
...
data_out1=static_solver_CTS_membrane_time_merge(data);
...
n_a_t=data_out1.n_a_out;          %nodal coordinate in every step
n_b_t=data_out1.n_b_out;          %nodal coordinate in every step

epsilon=data_out1.epsilon_out;
sigma_l=data_out1.sigma_l_out;    %sigma_l

E1_end=data_out1.E1_end_out;
T1_t=data_out1.T1_out; 
...
for i=1:10
    num=ceil(j(i)*size(n_a_t,2));
tenseg_plot_ori_membrane(reshape(n_a_t(:,num),3,[]),C_b,C_s,C_l,[],[],[],[],[30,30],[] ,[],Ca);
%  axis off;
end
%% save output data
if savedata==1
...
tenseg_video_ori_membrane(n_a_t,C_b,C_s,C_l,[],[],Ca,[],min(icrm,50),name,savevideo,[])
