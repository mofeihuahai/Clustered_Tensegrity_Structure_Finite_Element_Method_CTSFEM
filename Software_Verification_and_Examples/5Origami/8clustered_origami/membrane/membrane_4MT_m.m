...
C_l_in = [1,2;1,3;2,4;3,6;4,5;5,6;2,5;3,5;2,3];  % panel lines
...
M=1/12*kron((C_pn_bar'*diag(mass_p)*C_pn_bar+diag(diag(C_pn_bar'*diag(mass_p)*C_pn_bar))),eye(3)); % mass matrix of panel
...
gr_tc={[1,2,3]};     % number of elements in one group(for truss)
...
tenseg_plot_ori(reshape(n_t(:,num),3,[]),C_l,C_t,[],[],[],[],[30,30],[] ,[],Ca);
%  axis off;
end
%% save output data
if savedata==1
...
tenseg_video_ori(n_t,C_l,C_t,[],[],Ca,[],min(icrm,50),name,savevideo,[])
