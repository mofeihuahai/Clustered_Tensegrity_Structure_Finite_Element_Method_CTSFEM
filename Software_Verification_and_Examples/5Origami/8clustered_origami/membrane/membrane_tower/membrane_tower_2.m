...
% end
 ...
 N_max(:,1)=N(:,4);
 N_max(:,2)=N(:,5);   %把三个顶点赋值到对应位置
 N_max(:,3)=N(:,6);

num_x1=linspace(N_max(1,1),N_max(1,2),f);
num_x2=linspace(N_max(1,1),N_max(1,3),f);                     %把最外面的边长划分为f个（关于x）
num_x3=linspace(N_max(1,2),N_max(1,3),f);

num_y1=linspace(N_max(2,1),N_max(2,2),f);
num_y2=linspace(N_max(2,1),N_max(2,3),f);                     %把最外面的边长划分为f个（关于y）    
num_y3=linspace(N_max(2,2),N_max(2,3),f);

num_z1=linspace(N_max(3,1),N_max(3,2),f);
num_z2=linspace(N_max(3,1),N_max(3,3),f);                     %把最外面的边长划分为f个（关于z）    
num_z3=linspace(N_max(3,2),N_max(3,3),f);


for i=1:(f-2)
    N_max(1,(1+(1+(i+1))*(i+1)/2))=num_x1(1,1+i);
    N_max(1,(i^2+3*i+6)/2)=num_x2(1,1+i);              
 
    N_max(2,(1+(1+(i+1))*(i+1)/2))=num_y1(1,1+i);    %确定1-2和1-3边上的节点
    N_max(2,(i^2+3*i+6)/2)=num_y2(1,1+i); 

    N_max(3,(1+(1+(i+1))*(i+1)/2))=num_z1(1,1+i);
    N_max(3,(i^2+3*i+6)/2)=num_z2(1,1+i);
end

N_max(1,((f-2)^2+3*(f-2)+6)/2+1:(1+f)*f/2)=num_x3(2:(f-1));
N_max(2,((f-2)^2+3*(f-2)+6)/2+1:(1+f)*f/2)=num_y3(2:(f-1));     %确定2-3边上的节点
N_max(3,((f-2)^2+3*(f-2)+6)/2+1:(1+f)*f/2)=num_z3(2:(f-1));


for i=2:(f-2)
    num_x=linspace(N_max(1,(1+(1+(i+1))*(i+1)/2)),N_max(1,(i^2+3*i+6)/2),(i+1));
    num_y=linspace(N_max(2,(1+(1+(i+1))*(i+1)/2)),N_max(2,(i^2+3*i+6)/2),(i+1));
    num_z=linspace(N_max(3,(1+(1+(i+1))*(i+1)/2)),N_max(3,(i^2+3*i+6)/2),(i+1));

    N_max(1,(((i-1)^2+3*(i-1)+6)/2+1):((1+(i+1))*(i+1)/2))=num_x(2:i);                       
    N_max(2,(((i-1)^2+3*(i-1)+6)/2+1):((1+(i+1))*(i+1)/2))=num_y(2:i);                          %把中间的点按位置划分次数并赋值
    N_max(3,(((i-1)^2+3*(i-1)+6)/2+1):((1+(i+1))*(i+1)/2))=num_z(2:i);                        
