...
N_new=[];
for i=1:p-2
    N_f=N_initial([1 2 3],[1 i+1 i+2]); %根据几边形划分出对应三角形个数
    N_new_i=membrane_f2(N_f,f);   %划分后行的节点坐标
    N_new=[N_new,N_new_i];
end 
X=[N_initial,N_new];    %初始节点不变，划分后节点加上去
...
% end
% C_l_in(C_l_in==2)=-1;    %-1是一个不会出现的数，只是用来替换
% C_l_in(C_l_in==(4+(1+(f-1))*(f-1)/2))=2;      %将5,6列与底部两个调换
% C_l_in(C_l_in==-1)=(4+(1+(f-1))*(f-1)/2);        
% C_l_in(C_l_in==3)=-2;    %-2是一个不会出现的数，只是用来替换
% C_l_in(C_l_in==(3+(1+f)*f/2))=3;              %将5,6列与底部两个调换
