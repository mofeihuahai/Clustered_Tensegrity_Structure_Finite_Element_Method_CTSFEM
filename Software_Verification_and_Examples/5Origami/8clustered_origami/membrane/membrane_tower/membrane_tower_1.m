...
r=10; h=30; p=3; f=4;       % radius; height; number of edge;三角形边长划分数量

beta=180*(0.5-1/p); 	% rotation angle
for i=1:p               % nodal coordinate matrix N
    N(:,i)=3*r*[cos(2*pi*(i-1)/p),sin(2*pi*(i-1)/p),0];
end

for i=p+1:2*p
    N(:,i)=[3*r*cos(2*pi*(i-1)/p+beta*pi/180),3*r*sin(2*pi*(i-1)/p+beta*pi/180),h];
end
...
% end
 ...
 N_max=zeros(3,(1+f)*f/2);  %划分后三角形的节点坐标
 N_max(:,1)=N(:,4);
 N_max(:,(1+(1+(f-1))*(f-1)/2))=N(:,5);   %把三个顶点赋值到对应位置
 N_max(:,(1+f)*f/2)=N(:,6);

num_x1=linspace(N_max(1,1),N_max(1,(1+(1+(f-1))*(f-1)/2)),f);
num_x2=linspace(N_max(1,1),N_max(1,(1+f)*f/2),f);                     %把最外面的边长划分为f个（关于x）
num_x3=linspace(N_max(1,(1+(1+(f-1))*(f-1)/2)),N_max(1,(1+f)*f/2),f);

num_y1=linspace(N_max(2,1),N_max(2,(1+(1+(f-1))*(f-1)/2)),f);
num_y2=linspace(N_max(2,1),N_max(2,(1+f)*f/2),f);                     %把最外面的边长划分为f个（关于y）    
num_y3=linspace(N_max(2,(1+(1+(f-1))*(f-1)/2)),N_max(2,(1+f)*f/2),f);

num_z1=linspace(N_max(3,1),N_max(3,(1+(1+(f-1))*(f-1)/2)),f);
num_z2=linspace(N_max(3,1),N_max(3,(1+f)*f/2),f);                     %把最外面的边长划分为f个（关于z）    
num_z3=linspace(N_max(3,(1+(1+(f-1))*(f-1)/2)),N_max(3,(1+f)*f/2),f);


for i=1:f
    N_max(1,(1+(1+(i-1))*(i-1)/2))=num_x1(1,i);
    N_max(1,((1+i)*i/2))=num_x2(1,i);               %把上面划分的赋值到对应位置（关于x）
    N_max(1,((1+(f-1))*(f-1)/2+i))=num_x3(1,i);  

    N_max(2,(1+(1+(i-1))*(i-1)/2))=num_y1(1,i);
    N_max(2,((1+i)*i/2))=num_y2(1,i);               %把上面划分的赋值到对应位置（关于y）
    N_max(2,((1+(f-1))*(f-1)/2+i))=num_y3(1,i);

    N_max(3,(1+(1+(i-1))*(i-1)/2))=num_z1(1,i);
    N_max(3,((1+i)*i/2))=num_z2(1,i);               %把上面划分的赋值到对应位置（关于z）
    N_max(3,((1+(f-1))*(f-1)/2+i))=num_z3(1,i);

end

for i=2:(f-2)
    num_x=linspace(N_max(1,(1+(1+i)*i/2)),N_max(1,((1+(i+1))*(i+1)/2)),(i+1));
    num_y=linspace(N_max(2,(1+(1+i)*i/2)),N_max(2,((1+(i+1))*(i+1)/2)),(i+1));
    num_z=linspace(N_max(3,(1+(1+i)*i/2)),N_max(3,((1+(i+1))*(i+1)/2)),(i+1));
    N_max(1,((1+(1+i)*i/2):((1+(i+1))*(i+1)/2)))=num_x;                          %把中间的点按位置划分次数并赋值
    N_max(2,((1+(1+i)*i/2):((1+(i+1))*(i+1)/2)))=num_y;
    N_max(3,((1+(1+i)*i/2):((1+(i+1))*(i+1)/2)))=num_z;
end
N=[N([1 2 3],[1 2 3]),N_max];      %将划分后的节点坐标并入整体坐标内
...
for i=1:(f-1)
    ...
    for j=1:i 
        C_l_in_j(1+3*(j-1):(3*j),:)=[3+j+(1+(i-1))*(i-1)/2,3+j+(1+(i-1))*(i-1)/2+i;3+j+(1+(i-1))*(i-1)/2,3+j+(1+(i-1))*(i-1)/2+(i+1);3+j+(1+(i-1))*(i-1)/2+i,3+j+(1+(i-1))*(i-1)/2+i+1];
    end 
    C_l_in = [C_l_in;C_l_in_j];    
end

C_b_in = [1,(((1+(f-1))*(f-1)/2)+4);2,((1+f)*f/2+3);3,4];  %bar
C_s_in = [1,4;2,(((1+(f-1))*(f-1)/2)+4);3,((1+f)*f/2+3)];  %string
