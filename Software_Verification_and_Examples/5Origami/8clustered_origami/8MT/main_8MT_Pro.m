...
tf=0.5;                   % final time of dynamic simulation
...
width=0.1;
N=width*[1,3,1,3,0,2,4,1;0,0,2,2,2+sqrt(3),2+sqrt(3),2+sqrt(3),2+2*sqrt(3);0,0,0,0,0.1,0.05,0.1,0.15];       %nodal coordinate
%N=N+1e-4*rand(size(N));
C_b_in = [1,2;1,3;2,4;3,5;4,7;6,7;5,8;6,8];   %bar in boundary
C_h_in_1 = [5,6;3,6;4,6];      %bars in rotational hinge
C_h_in_2 = [3,4;2,3];              %bar in rigid hinge
C_h_in = [C_h_in_1;C_h_in_2];  %hinge in plane
C_s_in = [1,5;5,8;8,7;7,2];  %string in plane
C_in = [C_b_in;C_h_in];
...
Ca=[1,2,3,3,4,5;2,4,6,4,7,6;3,3,5,6,6,8];
...
%% Boundary constraints
 pinned_X=[1,2,3,4]'; pinned_Y=[1,2,3,4]'; pinned_Z=[1,2,3,4]';
[Ia,Ib,a,b]=tenseg_boundary(pinned_X,pinned_Y,pinned_Z,nn);
%% Group/Clustered information 
%generate group index
% gr=[];
gr={[14,15,16,17]};     % number of elements in one group
...
k_h(index_rh_in_h)=1e-1*1/12*E_c(index_rh).*l(index_rh)*thick^3;      % increase stiffness of rigid hinge
...
tenseg_plot_result(1:substep,l0_ct([14],:),{'string'},{'Substep','Rest length (m)'},'plot_coordinate.png',saveimg);
grid on;
...
%% plot member force 
tenseg_plot_result(1:substep,t_t([14],:),{'string'},{'Substep','Force (N)'},'plot_member_force.png',saveimg);
grid on;
%% Plot nodal coordinate curve X Y
tenseg_plot_result(1:substep,n_t([16,17,18,22,23,24],:),{'x6','y6','z6','x8','y8','z8'},{'Substep','Coordinate (m)'},'plot_coordinate.png',saveimg);
grid on;
%% plot hinge moment
tenseg_plot_result(1:substep,M_t,{'sft1','sft2','sft3','rgd1','rgd2'},{'Substep','Moment (N·m)'},'plot_hinge_moment.png',saveimg);


%% plot member force 
% Fhis=linspace(0,1,substep);
% 
% tenseg_plot_result2(Fhis,t_t([4,17],:),{'string','bar'}...
...
% 
% 
% tenseg_plot_result2(Fhis,l0_ct([14],:),{'string'}...
...
% 
% 
% tenseg_plot_result2(Fhis,M_t([1,2,3,4,5],:),{'sft1','sft2','sft3','rgd1','rgd2'}...
...
%% make video of the dynamic
icrm=size(n_t,2); 
name=['8MT_Pro_dynamic_G_s'];
% tenseg_video(n_t,C_b,C_s,[],min(substep,50),name,savevideo,R3Ddata);
% tenseg_video_slack(n_t,C_b,C_s,l0_ct,index_s,[],[],[],min(substep,50),name,savevideo,material{2})
tenseg_video_ori(n_t,[],C_s,C_h,C_rh,Ca,[],min(icrm,150),name,savevideo,[])

%% plot member force 
tenseg_plot_result(downsample(out_tspan,30),downsample(t_t([14],:),30),{'string'},{'Time (s)','Force (N)'},'plot_member_force.png',saveimg);
grid on;
%% Plot nodal coordinate curve X Y
% tenseg_plot_result(downsample(out_tspan,30),downsample(n_t([16,17,18,22,23,24],:),30),{'x6','y6','z6','x8','y8','z8'},{'Time (s)','Coordinate (m)'},'plot_coordinate.png',saveimg);
% grid on;

tenseg_plot_result(downsample(out_tspan,30),[downsample(n_t([16],:),30);downsample(n_t([17],:),30);downsample(n_t([18],:),30);downsample(n_t([22],:),30);downsample(n_t([23],:),30);downsample(n_t([24],:),30)],{'x6','y6','z6','x8','y8','z8'},{'Time (s)','Coordinate (m)'},'plot_coordinate.png',saveimg);
grid on;
