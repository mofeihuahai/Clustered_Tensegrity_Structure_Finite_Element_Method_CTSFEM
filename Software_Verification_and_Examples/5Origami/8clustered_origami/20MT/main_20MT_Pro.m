...
width=0.1;
N=width*[1,3,5,7,9,11,0,2,4,6,8,10,1,3,5,7,9,4.382,7.618,6,6;0,0,0,0,0,0,sqrt(3),sqrt(3),sqrt(3),sqrt(3),sqrt(3),sqrt(3),2*sqrt(3),2*sqrt(3),2*sqrt(3),2*sqrt(3),2*sqrt(3),-1.902,-1.902,-3.078,-1.377;0.03,0.01,0,0,0.01,0.03,0.09,0.07,0.05,0.05,0.07,0.09,0.15,0.12,0.1,0.12,0.15,0,0,0,0];       %nodal coordinate
...
%% Boundary constraints
 pinned_X=[3,4,18,19,20,21]'; pinned_Y=[3,4,18,19,20,21]'; pinned_Z=[3,4,18,19,20,21]';
...
k_h=1e1*1/12*E_c(index_h).*l(index_h)*thick^3;
k_h(index_rh_in_2)=1e-3*1/12*E_c(index_rh_2).*l(index_rh_2)*thick^3;      % 比较刚的hinge
k_h(index_rh_in_3)=1e2*1/12*E_c(index_rh_3).*l(index_rh_3)*thick^3;      % increase stiffness of rigid hinge
%% rest length (of truss), initial angle (of hinge)
l0=l;                     %rest length of truss
theta_0=pi*ones(n_h,1);     % initial angle of hinge
% theta_0=[pi;pi;pi]; % initial angle of hinge
...
ind_dnb=[]; dnb0=[];
%ind_dl0_c=[]; dl0_c=[];
ind_dl0_c=[42,41];dl0_c=[-1.79,-1.64]; %1
% ind_dl0_c=[41,42];dl0_c=[-1.6446,-1.8];   %2
...
%% plot member force 
Fhis=linspace(0,1,substep);

tenseg_plot_result2(Fhis,t_t([41,49],:),{'lower string','upper string'}...
...
tenseg_plot_result2(Fhis,l0_ct([41,42],:),{'lower string','upper string'}...
    ,{'Load factor','Length (m)'},'plot_member_force.png',saveimg,{'-or','-.vg','--xb','-^k'});
fig=gcf;
fig.Position(3:4)=[800,350];   %change fig size



tenseg_plot_result2(Fhis,M_t([1,2,3,15],:),{'rgd1','rgd2','rgd3','sft1'}...
    ,{'Load factor','Moment (N·m)'},'plot_member_force.png',saveimg,{'-or','-.vg','--xb','-^k','-.g'});
fig=gcf;
fig.Position(3:4)=[800,350];   %change fig size
...
% calculate external force and 
ind_w=[];w=[];
ind_dl0_c=[42,41];dl0_c=[-1.79,-1.64];
ind_theta_0=[]; dtheta_0=[];        % initial angel change with time

[w_t,l0_ct,theta_0t]=tenseg_load_prestress_CTS_ORI(tspan,ind_w,w,'ramp',ind_dl0_c,dl0_c,l0_c,ind_theta_0,dtheta_0,theta_0,gravity,[0;0;9.8],C,M_p,p);
