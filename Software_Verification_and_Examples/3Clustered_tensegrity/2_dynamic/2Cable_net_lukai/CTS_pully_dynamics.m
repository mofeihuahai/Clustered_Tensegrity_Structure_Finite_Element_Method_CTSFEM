...
tf=0.6;                   % final time of dynamic simulation
...
for i=1:p
    ...
    if i<=3
        ...
        if i>9 && i<=12
            r=2^(1/2)*sin(pi/4)/sin(9*pi/4-alpha)/2;
...
% end
dnb0=[0 0.15 0.3 0.3 0.15 0 0 0.15 0.3 0.3 0.15 0];    %抬升高度
...
% time step
if auto_dt
dt=pi/(max(omega)); 	% time step dt is 1/8 of the smallest period, guarantee convergence in solving ODE
end
tspan=0:dt:tf;
...
l0_c_ct=load('l0_c.txt');     %导入不变力时计算的十组原长
[w_t,l0_ct]=tenseg_loadprestress_CTS_contant(tspan,ind_w,w,'ramp',l0_c_ct,gravity,[0;0;0],C,mass);
...
%% plot member force 
tenseg_plot_result(out_tspan,t_t([1,2*p+1],:),{'1','2'},{'Time (s)','Force (N)'},'plot_member_force.png',saveimg);

%% Plot nodal coordinate curve X Y
tenseg_plot_result(out_tspan,n_t([3*4-2,3*4-1,3*4],:),{'4X','4Y','4Z'},{'Time (s)','Coordinate (m)'},'plot_coordinate.png',saveimg);
